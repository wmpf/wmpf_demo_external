...
class IotActivity : AppCompatActivity() {
    ...
    private fun doFacePay(view: View) {
        ...
        // javayhu 这里直接将 initWxFacePayInfo 传入的数据复制到这里
        val info = hashMapOf(
                ...
                "out_trade_no" to outTradeNo as Object, //"商户订单号"，须与调用支付接口时字段一致，该字段在在face_code_type为"1"时可不填，为"0"时必填
        ...
        WxPayFace.getInstance().getWxpayfaceCode(info, object : IWxPayfaceCallback() {
